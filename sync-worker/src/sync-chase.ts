...
import { waitWhileDbHot } from "./db-load.js";
 ...
 * If Postgres is hot (OCR spike), delay up to ~2 minutes first.
 */
export async function maybeChainNextDeltaSync(
  ...
  const { waitedMs, lastClients } = await waitWhileDbHot({
    label: "sync-chase",
  });
  if (waitedMs > 0) {
    console.log(
      `[sync-chase] delayed ${Math.round(waitedMs / 1000)}s (clients=${lastClients ?? "?"}) before next delta`
    );
  }
